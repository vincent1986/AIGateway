...
func TestResponsesFallsBackToNextRoute(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	t.Setenv("USERPROFILE", tmp)
	closeDB()
	...
	first := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		firstHits++
		http.Error(w, `{"error":{"code":"insufficient_quota"}}`, http.StatusTooManyRequests)
	}))
	defer first.Close()
	...
	if err := saveProvidersToDisk([]Provider{
		{
			ID: "p1", Name: "First", BaseURL: first.URL + "/v1", APIKey: "sk-1",
			Models: []ProviderModel{{ID: "shared-model", Enabled: true}},
		},
		{
			ID: "p2", Name: "Second", BaseURL: second.URL + "/v1", APIKey: "sk-2",
			Models: []ProviderModel{{ID: "shared-model", Enabled: true}},
		},
	}); err != nil {
		t.Fatal(err)
	...
	body := strings.NewReader(`{"model":"shared-model","input":"hi"}`)
	req := httptest.NewRequest(http.MethodPost, "/v1/responses", body)
	rr := httptest.NewRecorder()
	newProxyServer().handleResponses(rr, req)

	if rr.Code != http.StatusOK {
		t.Fatalf("status=%d body=%s", rr.Code, rr.Body.String())
	}
	if firstHits == 0 || secondHits == 0 {
		t.Fatalf("expected both routes to be tried, first=%d second=%d", firstHits, secondHits)
	}
	if !strings.Contains(rr.Body.String(), "fallback ok") {
		t.Fatalf("missing fallback response: %s", rr.Body.String())
	}
}
