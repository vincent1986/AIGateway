...
func (p *proxyServer) start() error {
	...
	// Claude Code uses Anthropic's Messages API rather than OpenAI chat.
	mux.HandleFunc("/v1/messages", p.handleAnthropicMessages)
	mux.HandleFunc("/messages", p.handleAnthropicMessages)
...
func (p *proxyServer) handleModels(w http.ResponseWriter, r *http.Request) {
	...
	for _, kind := range []ToolKind{ToolCodex, ToolClaude, ToolOpenClaw, ToolHarness, ToolGrok} {
		...
		seen[alias] = true
		data = append(data, item{ID: alias, Object: "model", OwnedBy: "aigateway"})
	}
...
func (p *proxyServer) forwardOpenAI(w http.ResponseWriter, r *http.Request, openAIPath string) {
	...
	for i, cand := range cands {
		...
		if stream {
			...
			if peekErr != nil {
				_ = resp.Body.Close()
				recordUsageFromSSE(acc.Bytes(), prov.Name, cand.UpstreamModel, openAIPath, resp.StatusCode)
				return
			}
			buf := make([]byte, 4096)
			for {
		...
		w.WriteHeader(resp.StatusCode)
		recordUsageFromPayload(upBody, prov.Name, cand.UpstreamModel, openAIPath, resp.StatusCode)
		_, _ = w.Write(upBody)
		return
	}
