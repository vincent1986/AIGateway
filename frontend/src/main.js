...
function appDebugLog(message) {
  try {
    const fn = window?.go?.main?.App?.AppDebugLog;
    if (typeof fn === "function") fn(String(message));
  } catch {
    // Debug logging must never block the UI action.
  }
}
...
function confirmAction(message, title = "") {
  return new Promise((resolve) => {
      ...
      <div class="modal-backdrop" id="modal-backdrop">
        <div class="modal" role="dialog" aria-modal="true">
          ...
          <div class="modal-body">
            ${lines.map((line) => `<p class="hint" style="margin:0 0 8px">${escapeHtml(line.trim())}</p>`).join("")}
          </div>
          <div class="modal-foot">
            <button class="btn" id="modal-cancel">${t("common.cancel")}</button>
            <button class="btn btn-primary" id="modal-ok">${t("common.confirm")}</button>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById("modal-close")?.addEventListener("click", () => done(false));
    document.getElementById("modal-cancel")?.addEventListener("click", () => done(false));
    document.getElementById("modal-ok")?.addEventListener("click", () => done(true));
    document.getElementById("modal-backdrop")?.addEventListener("click", (e) => {
      if (e.target.id === "modal-backdrop") done(false);
    });
  });
}
...
let proxyAlertReady = false;
const proxyAlertSeen = new Set();
...
function modelChoicesFor(kind) {
  ...
  for (const c of st?.candidates || []) {
    if (!c?.id || isProxyEntryModel(c.id) || seen.has(c.id)) continue;
    seen.add(c.id);
    list.push({ id: c.id, name: c.name || c.id, provider: c.provider || "", group: t("configs.groupInFile") });
  }
  for (const p of providers) {
    for (const m of p.models || []) {
      if (!m.enabled || !m.id || isProxyEntryModel(m.id) || seen.has(m.id)) continue;
      seen.add(m.id);
      list.push({ id: m.id, name: m.name || m.id, provider: "", group: p.name });
    }
  }
  if (st?.model && !isProxyEntryModel(st.model) && !seen.has(st.model)) {
...
async function loadToolConfigs(force = false) {
  ...
  try {
    ...
    for (const item of list || []) {
      ...
      pendingModel[item.kind] = item.proxyTargetModel || item.ProxyTargetModel || "";
...
async function applyToolStatus(item) {
  if (!item) return;
  toolConfigs[item.kind] = item;
  pendingModel[item.kind] = item.proxyTargetModel || item.ProxyTargetModel || pendingModel[item.kind] || "";
  pendingProvider[item.kind] = item.modelProvider || "";
  if (item.exists && item.path) {
    try {
      configPreview[item.kind] = await ReadConfigText(item.path);
...
// ─── Render ───
function render() {
  if (page !== "usage" && usageRefreshTimer) {
...
function renderAppCard(st) {
  ...
  const selectedModel = isProxyEntryModel(selectedModelRaw) ? "" : selectedModelRaw;
    ...
    <section class="config-card app-card" data-kind="${escapeAttr(kind)}">
      ...
      <div class="config-card-body">
        ...
        <div class="kv" style="margin-top:12px">
          ...
          <div class="value">${escapeHtml(selectedModel || t("configs.unset"))}</div>
        </div>
...
async function refreshProxyStatus() {
  ...
  try {
    ...
    observeProxyLogs(proxyStatus.logs);
...
function observeProxyLogs(logs) {
  const lines = Array.isArray(logs) ? logs : [];
  if (!proxyAlertReady) {
    lines.forEach((line) => proxyAlertSeen.add(line));
    proxyAlertReady = true;
    return;
  }
  for (const line of lines) {
    if (proxyAlertSeen.has(line)) continue;
    proxyAlertSeen.add(line);
    if (line.includes("全部渠道耗尽")) {
      toast(t("proxy.alertExhausted"), "err");
    } else if (line.includes("上游错误")) {
      toast(`${t("proxy.alertUpstream")}：${line}`, "err");
    }
  }
  if (proxyAlertSeen.size > 240) {
    const keep = new Set(lines);
    for (const line of proxyAlertSeen) {
      if (!keep.has(line)) proxyAlertSeen.delete(line);
    }
  }
}

function startProxyAlertMonitor() {
  if (proxyAlertTimer) clearInterval(proxyAlertTimer);
  proxyAlertTimer = setInterval(async () => {
    if (document.hidden || !hasBackend() || typeof GetProxyStatus !== "function") return;
    try {
      const next = normalizeProxyStatus(await GetProxyStatus());
      proxyStatus = next;
      observeProxyLogs(next.logs);
    } catch {
      // The proxy may be restarting; the next poll will retry.
    }
  }, 2000);
}
...
function bindConfigEvents() {
  ...
  document.querySelectorAll("[data-act='takeover']").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      const current = toolConfigs[kind] || {};
      appDebugLog(`takeover click kind=${kind} path=${current.path || ""} managed=${!!(current.managed || current.Managed)} hasTakeover=${!!(current.hasTakeoverBackup || current.HasTakeoverBackup)} disabled=${btn.disabled}`);
      configsBusy = kind;
      render();
      try {
        if (!hasBackend()) throw new Error(t("toast.runInWailsShort"));
        appDebugLog(`takeover backend start kind=${kind}`);
        const st = await InjectGateway(kind);
        appDebugLog(`takeover backend done kind=${kind} returnedPath=${st?.path || ""} provider=${st?.modelProvider || ""} managed=${!!(st?.managed || st?.Managed)}`);
        await applyToolStatus(st);
        toast(st.message || t("apps.takenOver"));
      } catch (e) {
        appDebugLog(`takeover backend error kind=${kind} error=${errMsg(e)}`);
        toast(errMsg(e), "err");
      } finally {
        configsBusy = "";
        render();
  ...
  document.querySelectorAll("[data-act='rollback-gw']").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const kind = btn.dataset.kind;
      const current = toolConfigs[kind] || {};
      appDebugLog(`rollback click kind=${kind} path=${current.path || ""} managed=${!!(current.managed || current.Managed)} hasTakeover=${!!(current.hasTakeoverBackup || current.HasTakeoverBackup)} hasDefault=${!!(current.hasDefaultBackup || current.HasDefaultBackup)} disabled=${btn.disabled}`);
      const name = toolConfigs[kind]?.name || kind;
      const confirmed = await confirmAction(t("confirm.restoreDefault", { name }), t("apps.rollback"));
      appDebugLog(`rollback confirm result kind=${kind} confirmed=${confirmed}`);
      if (!confirmed) return;
      appDebugLog(`rollback confirmed kind=${kind}`);
      configsBusy = kind;
      render();
      try {
        if (!hasBackend()) throw new Error(t("toast.runInWailsShort"));
        appDebugLog(`rollback backend start kind=${kind}`);
        const st = await RollbackGateway(kind);
        appDebugLog(`rollback backend done kind=${kind} returnedPath=${st?.path || ""} provider=${st?.modelProvider || ""} managed=${!!(st?.managed || st?.Managed)}`);
        await applyToolStatus(st);
        toast(st.message || t("toast.restored"));
      } catch (e) {
        appDebugLog(`rollback backend error kind=${kind} error=${errMsg(e)}`);
        toast(errMsg(e), "err");
      } finally {
        configsBusy = "";
        render();
  ...
  document.querySelectorAll("[data-act='model-select']").forEach((sel) => {
    sel.addEventListener("change", () => {
      ...
      pendingModel[kind] = isProxyEntryModel(sel.value) ? "" : sel.value;
  ...
  document.querySelectorAll("[data-act='apply-model']").forEach((btn) => {
    btn.addEventListener("click", async () => {
      const kind = btn.dataset.kind;
      const selected = document.querySelector(`[data-act='model-select'][data-kind='${kind}']`)?.value || pendingModel[kind] || "";
      const model = isProxyEntryModel(selected) ? "" : selected.trim();
      if (!model) return toast(t("toast.selectModel"), "err");
      configsBusy = kind;
      render();
      try {
        if (!hasBackend()) throw new Error(t("toast.runInWailsShort"));
        const st = await SetAppProxyModel(kind, model);
        await applyToolStatus(st);
