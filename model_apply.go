...
func (a *App) ApplyToolModel(req ModelApplyRequest) (ToolConfigStatus, error) {
	...
	if d == nil {
		return ToolConfigStatus{}, fmt.Errorf("未找到应用驱动: %s", req.Kind)
	}
	...
	case ToolCodex:
		...
		applied, applyErr := d.ApplyModel([]byte(content), path, ModelInjection{
			Model: model, Provider: providerID, BaseURL: writeBase, APIKey: apiKey, Name: displayName,
		})
		if applyErr != nil {
			return ToolConfigStatus{}, applyErr
		}
		next = string(applied)
		// Keep the credential reference in the Codex config only. Model changes
		// must not mutate the user's global shell or launch environment.
		if writeBase != "" && isLocalProxyURL(writeBase) {
			autoBakMsg += "；base_url→本地代理"
		}
	case ToolClaude:
		// Normalize DeepSeek values before passing the format-specific write to the driver.
		...
		applied, applyErr := d.ApplyModel([]byte(content), path, ModelInjection{
			Model: model, Provider: providerID, BaseURL: baseURL, APIKey: apiKey, Name: displayName,
		})
		if applyErr != nil {
			return ToolConfigStatus{}, applyErr
		}
	...
	if err := validateToolConfigContent(k, path, next, model); err != nil {
		return ToolConfigStatus{}, err
	}
	if err := writeFileAtomic(path, next); err != nil {
		return ToolConfigStatus{}, err
	}
	if err := validateToolConfigWrite(k, path, model); err != nil {
		return ToolConfigStatus{}, err
	}
...
func sanitizeGenericProviderID(providerID, baseURL, displayName string) string {
...
func injectGenericModelGateway(configPath, baseURL, apiKey, model, providerID string) error {
	if strings.HasSuffix(strings.ToLower(configPath), ".json") {
		...
		if err != nil || len(raw) == 0 {
			...
			return fmt.Errorf("解析 JSON 配置失败（为避免覆盖现有配置，已停止写入）: %w", err)
		}
		if baseURL != "" {
			root["baseUrl"] = baseURL
			root["base_url"] = baseURL
			root["apiBaseUrl"] = baseURL
		}
		if model != "" {
			root["model"] = model
	...
	var err error
	content, err = applyHarnessYAML(content, baseURL, apiKey, model, providerID)
	if err != nil {
		return err
	}
	if err := os.MkdirAll(filepath.Dir(configPath), 0o755); err != nil {
...
// applyHarnessYAML only edits recognized top-level Harness fields. We do not
// guess nested provider schemas: an unknown non-empty YAML file is rejected
// instead of being reported as successfully configured but ignored by the app.
func applyHarnessYAML(content, baseURL, apiKey, model, providerID string) (string, error) {
	if strings.TrimSpace(content) != "" && strings.TrimSpace(content) != "# AIGateway managed" {
		hasKnown := false
		for _, line := range strings.Split(content, "\n") {
			...
			if ok && isHarnessTopLevelKey(strings.TrimSpace(key)) {
				hasKnown = true
				break
			}
		}
		if !hasKnown {
			return "", fmt.Errorf("无法识别 Harness YAML 配置结构：只支持已识别的顶层 model/provider/base_url/api_key 字段")
		}
	}
	var found bool
	if baseURL != "" {
		content, found = upsertYAMLTopLevelKey(content, "base_url", baseURL)
		if !found {
			content, found = upsertYAMLTopLevelKey(content, "baseUrl", baseURL)
		}
		if !found {
			content = appendYAMLTopLevelKey(content, "base_url", baseURL)
		}
	}
	if apiKey != "" {
		content, found = upsertYAMLTopLevelKey(content, "api_key", apiKey)
		if !found {
			content = appendYAMLTopLevelKey(content, "api_key", apiKey)
		}
	}
	if model != "" {
		content, found = upsertYAMLTopLevelKey(content, "model", model)
		if !found {
			content = appendYAMLTopLevelKey(content, "model", model)
		}
	}
	if providerID != "" {
		content, found = upsertYAMLTopLevelKey(content, "provider", providerID)
		if !found {
			content = appendYAMLTopLevelKey(content, "provider", providerID)
		}
	}
	return content, nil
}

func isHarnessTopLevelKey(key string) bool {
	switch key {
	case "model", "provider", "base_url", "baseUrl", "api_key":
		return true
	default:
		return false
	}
...
func upsertYAMLTopLevelKey(content, key, value string) (string, bool) {
	lines := strings.Split(content, "\n")
	prefix := key + ":"
	for i, line := range lines {
		if len(line) != len(strings.TrimLeft(line, " \t")) || !strings.HasPrefix(line, prefix) {
			continue
		}
...
func appendYAMLTopLevelKey(content, key, value string) string {
...
//  6. Preserve wire_api because it selects the upstream Codex protocol.
func buildProviderBlock(id, name, baseURL, apiKey string, existing *providerKV) string {
	...
	// Preferred field order for readability while retaining protocol metadata.
	preferred := []string{"name", "base_url", "wire_api", "env_key", "api_key", "requires_openai_auth"}
	var out []string
