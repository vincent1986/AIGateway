...
func TestApplyClaudeModelSwitch(t *testing.T) {
	...
	if env["ANTHROPIC_CUSTOM_MODEL_OPTION"] != "gpt-test" {
		t.Fatalf("custom model=%v", env["ANTHROPIC_CUSTOM_MODEL_OPTION"])
	}
}
...
func TestApplyClaudeDeepSeekOfficial(t *testing.T) {
	...
	if env["ANTHROPIC_CUSTOM_MODEL_OPTION"] != deepSeekDefaultPro {
		t.Fatalf("custom model=%v", env["ANTHROPIC_CUSTOM_MODEL_OPTION"])
	}
...
func TestApplyToolModelMissingConfigCanRestoreToMissing(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("HOME", tmp)
	t.Setenv("USERPROFILE", tmp)
	closeDB()
	...
	path := filepathJoin(tmp, ".harness", "config.yaml")
	a := NewApp()
	if _, err := a.ApplyToolModel(ModelApplyRequest{
		Kind:     "harness",
		Path:     path,
		Model:    "model-a",
		Provider: "provider-a",
	...
	st, err := a.RestoreDefaultConfig("harness")
	if err != nil {
		t.Fatal(err)
	}
	if st.Exists {
		t.Fatalf("expected restored missing state: %+v", st)
	}
