...
面向 AIGateway 用户的高频问答。英文摘要附在各节末尾。  
更多排错见 [故障排除](Troubleshooting)；上手见 [快速开始](Getting-Started)。
...
> **EN:** A local multi-provider AI gateway: track usage, switch cheaper vendors, one-time tool takeover, virtual model groups with failover.
...
把 **不同厂家上的同名/等价模型** 聚合成一组（例如官方 DeepSeek + 硅基流动的同一模型名）。客户端只写一个模型名，网关按优先级选通道。

### Q14. Failover 何时触发？

上游返回如 **429 限流**、**额度/配额类错误**、**账户异常（如 401）** 等时，网关会按队列尝试下一顺位厂家。全部失败则返回 `model_group_all_exhausted`（OpenAI 风格错误 JSON）。

### Q15. 流式对话中途会不会切换厂家？

**有限制**：首包 / HTTP 层错误可切换；**已经开始向客户端推流后**无法无感中途换通道。详见 [故障排除](Troubleshooting)。
...
当前卡片包括：**ChatGPT（Codex）**、**Claude Code**、**OpenClaw**、**Harness**。其它 OpenAI 兼容客户端可手动填网关 Base URL。

### Q18. 一键接管会做什么？

定位工具配置文件 → **备份** → 将 `base_url`（及必要 provider 字段）指向本地网关。写入成功后，模型调度交给 AIGateway。
...
应通过 `models.providers.aigateway` 等方式接入网关；仅改根级错误字段可能导致未真正走 AIGateway。一键接管会按官方推荐结构写入。

### Q21. Codex 报保留 ID 或缺少 aigateway_api_key？

见 [故障排除](Troubleshooting)：

- 保留 ID：`openai` / `ollama` 改为 `openai-custom` / `ollama-local`  
- Key：使用 `api_key = "aigateway"`，不要依赖未设置的 env
...
应用内 **Token 统计** 页；底层写入 SQLite（`aigateway.db`）。

### Q23. 数据文件在哪？

见 [数据与路径](Data-Paths)。核心：`~/.codex-manager/aigateway.db`。

### Q24. 会把 API Key 上传到 GitHub 吗？

不会。Key 仅保存在本机配置/库中。提 Issue 时请自行打码。
...
- 流式 **中途** Failover 有限  
- 模型看板部分「套餐金额 / 已应用工具」字段可能仍在完善  
- 细节见仓库 `docs/PRD_V2_CHECKLIST.md` 与 Release Notes
...
使用 Issue 模板：https://github.com/vincent1986/AIGateway/issues/new/choose
...
文档随版本迭代，若与最新 UI 不一致，以 [最新 Release](https://github.com/vincent1986/AIGateway/releases) 为准。
