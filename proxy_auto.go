...
//   - provider protocol metadata such as wire_api is preserved
//
...
func (a *App) syncCodexProviderBases(list []Provider) (string, error) {
	...
	for _, p := range list {
		...
		// ensure provider block exists while preserving protocol metadata
...
// stripAllWireAPI is retained for compatibility with older callers. wire_api
// is valid protocol metadata and must never be removed during synchronization.
